...
import 'package:ai_chat_chat_client/services/extensions/client_download_content_extension.dart';
...
class MxcImage extends ConsumerStatefulWidget {
  final Uri? uri;
  final Event? event;
  final double? width;
  final double? height;
  ...
  const MxcImage({
    ...
    this.animated = false,
    this.animationDuration = AIChatChatThemes.animationDuration,
    this.retryDuration = const Duration(seconds: 2),
    this.animationCurve = AIChatChatThemes.animationCurve,
  ...
  @override
  ConsumerState<MxcImage> createState() => _MxcImageState();
}

class _MxcImageState extends ConsumerState<MxcImage> {
  ...
  Future<void> _load() async {
    final Client client =
        widget.client ?? widget.event?.room.client ?? ref.read(clientProvider);
